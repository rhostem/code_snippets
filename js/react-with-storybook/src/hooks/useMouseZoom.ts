...
export default function useMouseZoom({
  imageWrapperRef,
}: {
  ...
  const [zoomedWrapperSize, setZoomedWrapperSize] = useState({
    width: 0,
    height: 0,
  })
  const isPanEnabled = useMemo(() => isMouseEventActive && zoom > 1, [
    isMouseEventActive,
    zoom,
  ])
  ...
  const handleMouseMove = useCallback(
    e => {
      if (isPanEnabled && imageWrapperRef.current) {
        ...
        let nextPanX =
          -1 * (initialPos.current.x - e.clientX) + lastDistance.current.x
        let nextPanY =
          -1 * (initialPos.current.y - e.clientY) + lastDistance.current.y

        nextPanX =
          nextPanX > 0
            ? Math.min(maxPanX, nextPanX)
            : Math.max(-1 * maxPanX, nextPanX)
        nextPanY =
          nextPanY > 0
            ? Math.min(maxPanY, nextPanY)
            : Math.max(-1 * maxPanY, nextPanY)
    ...
    [
      isPanEnabled,
      imageWrapperRef,
      zoomedWrapperSize.width,
      zoomedWrapperSize.height,
    ]
  )
