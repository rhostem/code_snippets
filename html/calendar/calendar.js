...
function updateCalendar() {
  ...
  for (var weekIdx = 0; weekIdx < WEEK_LEN; weekIdx++) {
    for (var dayIdx = 0; dayIdx < DAY_LEN; dayIdx++) {
      ...
      `;
      cellElm.className = `calendar-day ${!isSameMonth ? 'is-notCurrentMonth' : ''}`;
      ...
      if (day.appointMent) {
        ...
        const y = weekIdx;
        const x = dayIdx;
        cellElm.getElementsByClassName('deleteAppointment')[0].addEventListener('click', (e) => {
          e.stopPropagation();
        ...
        cellElm.getElementsByClassName('editAppointment')[0].addEventListener('click', (e) => {
          e.stopPropagation();
          const text = prompt('일정을 수정하세요', calendar.grid[y][x].appointMent);
  ...
  for (var weekIdx = 0; weekIdx < WEEK_LEN; weekIdx++) {
    ...
    for (var dayIdx = 0; dayIdx < DAY_LEN; dayIdx++) {
      calendar[weekIdx][dayIdx] = {
        date: new Date(firstDayOfCalMilliSec + A_DAY_MILLI_SEC * (weekIdx * 7 + dayIdx)),
        appointMent: null,
      }
    }
  }
...
function getFirstDayOfCalendar(year, month) {
  ...
  if (firstDay !== 0) {
    ...
    firstDayOfCalendar = new Date(firstDayOfMonthVal - A_DAY_MILLI_SEC * (firstDay));
  }
